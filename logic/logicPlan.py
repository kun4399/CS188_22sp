...
def sentence1() -> Expr:
    ...
    B = Expr('B')
    C = Expr('C')
    result = [A | B, ~A % (~B | C), disjoin([~A, ~B, C])]
...
def sentence3() -> Expr:
    ...
    result = [PacmanAlive_1 % ((PacmanAlive_0 & ~PacmanKilled_0) | (~PacmanAlive_0 & PacmanBorn_0)),
...
def findModel(sentence: Expr) -> Dict[Expr, bool]:  # 这个就是model的数据类型
    ...
    # to convert the input sentence into Conjunctive Normal Form (the form required by the SAT solver),
    # and passes it to the SAT solver
    return pycoSAT(cnf_sentence)


def findModelCheck() -> Dict[Any, bool]:  # 这个方法好像是用来将大写字母转换成小写字母的
    ...
    class dummyClass:
        ...
        def __repr__(self):  # 这个是repr方法，返回一个对象的string形式，这个方法是在print的时候调用的
